import flatOne from './assets/picturesShoes/flatOne.png';
import flatTwo from './assets/picturesShoes/flatTwo.png';
import flatThree from './assets/picturesShoes/flatThree.png';

import sneakerOne from './assets/picturesShoes/sneakerOne.png';
import sneakerTwo from './assets/picturesShoes/sneakerTwo.png';
import sneakerThree from './assets/picturesShoes/sneakerThree.png';

import courtOne from './assets/picturesShoes/courtOne.png';
import courtTwo from './assets/picturesShoes/courtTwo.png';
import courtThree from './assets/picturesShoes/courtThree.png';

import chelseaOne from './assets/picturesShoes/chelseaOne.png';
import chelseaTwo from './assets/picturesShoes/chelseaTwo.png';
import chelseaThree from './assets/picturesShoes/chelseaThree.png';

import funOne from './assets/picturesShoes/funOne.png';
import funTwo from './assets/picturesShoes/funTwo.png';
import funThree from './assets/picturesShoes/funThree.png';

import bootOne from './assets/picturesShoes/bootsOne.png';
import bootTwo from './assets/picturesShoes/bootTwo.png';
import bootThree from './assets/picturesShoes/bootThree.png';

import whiteOne from './assets/picturesShoes/whiteOne.png';
import whiteTwo from './assets/picturesShoes/whiteTwo.png';
import whiteThree from './assets/picturesShoes/whiteThree.png';

import bikerOne from './assets/picturesShoes/bikerOne.png';
import bikerwo from './assets/picturesShoes/bikerTwo.png';
import bikerThree from './assets/picturesShoes/bikerThree.png';

import redOne from './assets/picturesShoes/redOne.png';
import redTwo from './assets/picturesShoes/redTwo.png';
import redThree from './assets/picturesShoes/redThree.png';

import furOne from './assets/picturesShoes/furOne.png';
import furTwo from './assets/picturesShoes/furTwo.png';
import furThree from './assets/picturesShoes/furThree.png';

import metalOne from './assets/picturesShoes/metalOne.png';
import metalTwo from './assets/picturesShoes/metalTwo.png';
import metalhree from './assets/picturesShoes/metalThree.png';

import platformOne from './assets/picturesShoes/platformOne.png';
import platformTwo from './assets/picturesShoes/platformTwo.png';
import platformThree from './assets/picturesShoes/platformThree.png';

import lepardOne from './assets/picturesShoes/lepardOne.png';
import lepardTwo from './assets/picturesShoes/lepardTwo.png';
import lepardThree from './assets/picturesShoes/lepardThree.png';

import overOne from './assets/picturesShoes/overOne.png';
import overTwo from './assets/picturesShoes/overTwo.png';
import overThree from './assets/picturesShoes/overThree.png';

import kittenOne from './assets/picturesShoes/kittenOne.png';
import kittenTwo from './assets/picturesShoes/kittenTwo.png';
import kittenThree from './assets/picturesShoes/kittenThree.png';

import puffyOne from './assets/picturesShoes/puffyOne.png';
import puffyTwo from './assets/picturesShoes/puffyTwo.png';
import puffyThree from './assets/picturesShoes/puffyThree.png';

import summerOne from './assets/picturesShoes/summerOne.png';
import summerTwo from './assets/picturesShoes/summerTwo.png';
import summerThree from './assets/picturesShoes/summerThree.png';
import summerFour from './assets/picturesShoes/summerFour.png';
import summerFive from './assets/picturesShoes/summerFive.png';
import summerSix from './assets/picturesShoes/summerSix.png';
import summerSeven from './assets/picturesShoes/summerSeven.png';


export const dataShop = [
    {
        id: 1,
        name: "Burgundy-loafers",
        title: "Burgundy loafers",
        price: "200",
        text: "Sed id quam est.",
        searchTerm: ["flats", "red"],
        imageShoe: flatOne,

    },

    {
        id: 2,
        name: "Leather-ballet-flats",
        title: "Leather ballet flats",
        price: "150",
        text: "Consectetur adipiscing elit.",
        searchTerm: ["flats", "brown"],
        imageShoe: flatTwo,
    },

    {
        id: 3,
        name: "Red-stylish-sandals",
        title: "Red stylish sandals",
        price: "275",
        text: "Mauris.",
        searchTerm: ["trending", "red", "heels"],
        imageShoe: funOne
    },

    {
        id: 4,
        name: "Blue-sneakers",
        title: "Blue sneakers",
        price: "125",
        text: "Lorem ipsum dolor sit amet.",
        searchTerm: ["sneakers", "flats", "trending"],
        imageShoe: sneakerOne
    },

    {
        id: 5,
        name: "Stylish-boots",
        title: "Stylish boots",
        price: "750",
        text: "Lorem ipsum dolor sit amet.",
        searchTerm: ["boots", "brown", "heels", "trending"],
        imageShoe: bootTwo
    },

    {
        id: 6,
        name: "Black-leather-sneakers",
        title: "Black leather sneakers",
        price: "225",
        text: "Consectetur adipiscing elit.",
        searchTerm: ["sneakers", "flats"],
        imageShoe: sneakerThree
    },

    {
        id: 7,
        name: "Suede-pumps",
        title: "Suede pumps",
        price: "475",
        text: "Lorem ipsum dolor sit amet.",
        searchTerm: ["heels", "brown"],
        imageShoe: courtOne
    },

    {
        id: 8,
        name: "Leather-boots",
        title: "Leather boots",
        price: "270",
        text: "Lorem ipsum dolor sit amet, consectetur.",
        searchTerm: ["boots", "white", "flats"],
        imageShoe: whiteTwo
    },

    {
        id: 9,
        name: "Black-mid-heels",
        title: "Black mid heels",
        price: "325",
        text: "In quis nulla at nibh tempus tempus. Sed id quam est. Mauris.",
        searchTerm: ["heels", "black"],
        imageShoe: courtThree
    },

    {
        id: 10,
        name: "Black-Chelsea-boots",
        title: "Black Chelsea boots",
        price: "340",
        text: "Lorem ipsum dolor sit amet, consectetur.",
        searchTerm: ["boots", "black", "flats"],
        imageShoe: chelseaOne
    },

    {
        id: 11,
        name: "Sparkly-heels",
        title: "Sparkly heels",
        price: "355",
        text: "Lorem ipsum dolor sit amet.",
        searchTerm: ["trending", "heels"],
        imageShoe: funTwo

    },

    {
        id: 12,
        name: "White-Chelsea-boots",
        title: "White Chelsea boots",
        price: "350",
        text: "Consectetur adipiscing elit.",
        searchTerm: ["boots", "white", "flats", "trending"],
        imageShoe: chelseaThree,
    },

    {
        id: 13,
        name: "Classy-loafers",
        title: "Classy loafers",
        price: "350",
        text: "Mauris.",
        searchTerm: ["flats", "brown", "trending"],
        imageShoe: flatThree
    },

    {
        id: 14,
        name: "Black-leather-boots",
        title: "Black leather boots",
        price: "550",
        text: "Sed id quam est.",
        searchTerm: ["boots", "black", "flats"],
        imageShoe: chelseaTwo,
    },

    {
        id: 15,
        name: "Lepard-print-heels",
        title: "Lepard print heels",
        price: "485",
        text: "In quis nulla at nibh tempus tempus.",
        searchTerm: ["heels"],
        imageShoe: funThree,
    },

    {
        id: 16,
        name: "Leather-long-boots",
        title: "Leather long boots",
        price: "650",
        text: "Consectetur adipiscing elit.",
        searchTerm: ["boots", "black", "flats"],
        imageShoe: bootOne
    },

    {
        id: 17,
        name: "Grey-sneakers",
        title: "Grey sneakers",
        price: "165",
        text: "In quis nulla at nibh tempus tempus.",
        searchTerm: ["sneakers", "flats"],
        imageShoe: sneakerTwo,
    },

    {
        id: 18,
        name: "Brown-leather-boots",
        title: "Brown leather boots",
        price: "425",
        text: "Sed id quam est. Mauris.",
        searchTerm: ["boots", "brown", "heels"],
        imageShoe: bootThree
    },

    {
        id: 19,
        name: "Cream-chelsea-boots",
        title: "Cream chelsea boots",
        price: "235",
        text: "In quis nulla at nibh tempus tempus. Sed id quam est. Mauris.",
        searchTerm: ["boots", "white", "flats"],
        imageShoe: whiteOne
    },

    {
        id: 20,
        name: "High-heels",
        title: "High heels",
        price: "425",
        text: "Sed id quam est. Mauris.",
        searchTerm: ["heels", "brown"],
        imageShoe: courtTwo
    },

    {
        id: 21,
        name: "White-combat-boots",
        title: "White combat boots",
        price: "200",
        text: "Sed id quam est.",
        searchTerm: ["boots", "white", "flats"],
        imageShoe: whiteThree

    },

    {
        id: 22,
        name: "Red-slingbacks",
        title: "Red slingbacks",
        price: "360",
        text: "In quis nulla at nibh tempus tempus.",
        searchTerm: ["red", "heels"],
        imageShoe: redOne,
    },

    {
        id: 23,
        name: "Brown-biker-boots",
        title: "Brown biker boots",
        price: "275",
        text: "Mauris.",
        searchTerm: ["boots", "brown", "flats"],
        imageShoe: bikerwo
    },

    {
        id: 24,
        name: "Yellow-slides",
        title: "Yellow slides",
        price: "250",
        text: "Sed id quam est. Mauris.",
        searchTerm: ["trending", "heels"],
        imageShoe: furOne
    },

    {
        id: 25,
        name: "Black-cowboy-boots",
        title: "Black cowboy boots",
        price: "350",
        text: "Consectetur adipiscing elit.",
        searchTerm: ["boots", "black", "flats"],
        imageShoe: bikerOne,
    },

    {
        id: 26,
        name: "Red-leather-loafers",
        title: "Red leather loafers",
        price: "325",
        text: "Consectetur adipiscing elit.",
        searchTerm: ["red", "flats"],
        imageShoe: redTwo
    },

    {
        id: 27,
        name: "Plaid-brown-shoes",
        title: "Plaid brown shoes",
        price: "250",
        text: "In quis nulla at nibh tempus tempus.",
        searchTerm: ["brown", "heels"],
        imageShoe: kittenTwo
    },

    {
        id: 28,
        name: "Black-biker-boots",
        title: "Black biker boots",
        price: "355",
        text: "Lorem ipsum dolor sit amet.",
        searchTerm: ["boots", "black", "flats"],
        imageShoe: bikerThree

    },

    {
        id: 29,
        name: "Lepard-heels",
        title: "Lepard heels",
        price: "340",
        text: "Sed id quam est. Mauris.",
        searchTerm: ["heels", "bestsellers"],
        imageShoe: lepardTwo
    },

    {
        id: 30,
        name: "White-puffy-boots",
        title: "White puffy boots",
        price: "435",
        text: "In quis nulla at nibh tempus tempus. Sed id quam est. Mauris.",
        searchTerm: ["white", "winter", "boots"],
        imageShoe: puffyThree
    },

    {
        id: 31,
        name: "Gold-ballet-flats",
        title: "Gold ballet flats",
        price: "200",
        text: "Sed id quam est.",
        searchTerm: ["trending", "flats"],
        imageShoe: metalOne

    },

    {
        id: 32,
        name: "Pink-leather-flats",
        title: "Pink leather flats",
        price: "225",
        text: "Lorem ipsum dolor sit amet.",
        searchTerm: ["summer", "flats"],
        imageShoe: summerFour
    },

    {
        id: 33,
        name: "Silver-loafers",
        title: "Silver loafers",
        price: "275",
        text: "Mauris.",
        searchTerm: ["trending", "flats"],
        imageShoe: metalhree
    },

    {
        id: 34,
        name: "Platform-boots",
        title: "Platform boots",
        price: "355",
        text: "Lorem ipsum dolor sit amet.",
        searchTerm: ["trending", "boots"],
        imageShoe: platformOne
    },

    {
        id: 35,
        name: "Brown-platform-boots",
        title: "Brown platform boots",
        price: "250",
        text: "In quis nulla at nibh tempus tempus.",
        searchTerm: ["trending", "brown", "boots"],
        imageShoe: platformTwo
    },

    {
        id: 36,
        name: "Purple-boots",
        title: "Purple boots",
        price: "375",
        text: "Consectetur adipiscing elit.",
        searchTerm: ["bestsellers", "boots"],
        imageShoe: platformThree
    },

    {
        id: 37,
        name: "Lepard-booties",
        title: "Lepard booties",
        price: "545",
        text: "Lorem ipsum dolor sit amet.",
        searchTerm: ["heels", "boots"],
        imageShoe: lepardOne
    },

    {
        id: 38,
        name: "Winter-boots",
        title: "Winter boots",
        price: "550",
        text: "In quis nulla at nibh tempus tempus. Sed id quam est. Mauris.",
        searchTerm: ["trending","boots"],
        imageShoe: furTwo
    },

    {
        id: 39,
        name: "Lepard-loafers",
        title: "Lepard loafers",
        price: "275",
        text: "In quis nulla at nibh tempus tempus. Sed id quam est. Mauris.",
        searchTerm: ["trending" , "flats"],
        imageShoe: lepardThree
    },


    {
        id: 40,
        name: "Long-cowboy-boots",
        title: "Long cowboy boots",
        price: "600",
        text: "Sed id quam est.",
        searchTerm: ["brown", "boots",],
        imageShoe: overOne,

    },

    {
        id: 41,
        name: "Suede-over-knee-boots",
        title: "Suede over knee boots",
        price: "540",
        text: "Consectetur adipiscing elit.",
        searchTerm: ["brown", "boots", "flats"],
        imageShoe: overTwo,
    },

    {
        id: 42,
        name: "Black-over-knee-boots",
        title: "Black over knee boots",
        price: "575",
        text: "Mauris.",
        searchTerm: ["black", "boots", "heels"],
        imageShoe: overThree
    },

    {
        id: 43,
        name: "White-leather-boots",
        title: "White leather boots",
        price: "355",
        text: "Lorem ipsum dolor sit amet.",
        searchTerm: ["white", "boots", "heels"],
        imageShoe: kittenOne
    },

    {
        id: 44,
        name: "Red-heels",
        title: "Red heels",
        price: "300",
        text: "Lorem ipsum dolor sit amet.",
        searchTerm: ["red", "heels"],
        imageShoe: redThree
    },

    {
        id: 45,
        name: "Summer-heels",
        title: "Summer heels",
        price: "250",
        text: "Consectetur adipiscing elit.",
        searchTerm: ["brown", "summer", "heels"],
        imageShoe: kittenThree
    },

    {
        id: 46,
        name: "Platform-puffy-boots",
        title: "Platform puffy boots",
        price: "370",
        text: "Lorem ipsum dolor sit amet.",
        searchTerm: ["winter", "boots", "black"],
        imageShoe: puffyOne
    },

    {
        id: 47,
        name: "Show-boots",
        title: "Show boots",
        price: "420",
        text: "Sed id quam est. Mauris.",
        searchTerm: ["winter", "boots"],
        imageShoe: puffyTwo
    },

    {
        id: 48,
        name: "Furry-heels",
        title: "Furry heels",
        price: "350",
        text: "Lorem ipsum dolor sit amet, consectetur.",
        searchTerm: ["trending", "heels", "brown"],
        imageShoe: furThree
    },

    {
        id: 49,
        name: "Red-sandals",
        title: "Red sandals",
        price: "250",
        text: "Sed id quam est.",
        searchTerm: ["red", "summer", "heels"],
        imageShoe: summerOne,

    },

    {
        id: 50,
        name: "White-leather-sneakers",
        title: "White leather sneakers",
        price: "320",
        text: "Consectetur adipiscing elit.",
        searchTerm: ["white", "flats"],
        imageShoe: summerTwo,
    },

    {
        id: 51,
        name: "Red-Espadrilles",
        title: "Red Espadrilles",
        price: "375",
        text: "Mauris.",
        searchTerm: ["red", "summer", "heels"],
        imageShoe: summerThree
    },

    {
        id: 52,
        name: "Silver-boots",
        title: "Silver boots",
        price: "475",
        text: "Consectetur adipiscing elit.",
        searchTerm: ["trending", "heels", "boots"],
        imageShoe: metalTwo
    },

    {
        id: 53,
        name: "Summer-suede-loafers",
        title: "Summer suede loafers",
        price: "300",
        text: "In quis nulla at nibh tempus tempus.",
        searchTerm: ["summer", "flats"],
        imageShoe: summerFive
    },

    {
        id: 54,
        name: "Leather-slides",
        title: "Leather slides",
        price: "150",
        text: "Consectetur adipiscing elit.",
        searchTerm: ["trending", "summer", "flats"],
        imageShoe: summerSix
    },

    {
        id: 55,
        name: "Fishermen-sandals",
        title: "Fishermen sandals",
        price: "320",
        text: "Lorem ipsum dolor sit amet.",
        searchTerm: ["trending", "summer", "black"],
        imageShoe: summerSeven
    },

]